<template>
  <div>
    <b-card :title="business.name" class="mb-5">
      <div v-if="editing">
        <b-form-textarea
          id="textarea"
          v-model="description"
        ...
        <b-form-group label-cols-sm="4" label="Address: " label-for="address">
          <b-form-input
            id="address"
            v-model="address"
        ...
        <b-form-group label-cols-sm="4" label="Phone: " label-for="phone">
          <b-form-input
            id="phone"
            v-model="phone"
        ...
        <b-form-group label-cols-sm="4" label="Email: " label-for="email">
          <b-form-input
            id="email"
            v-model="email"
        ...
        <div class="edit-info">
          <b-row class="mt-4">
            <b-button class="ml-3" v-on:click="cancelChanges">
              Cancel Changes</b-button
            >
            <b-button class="ml-2" v-on:click="saveChanges"
      ...
      <div v-else>
        ...
        <div v-if="correctBusiness">
          <b-button class="mt-4" v-on:click="editBusinessInfo"
            >Edit business info</b-button
          >
        </div>
      </div>
    </b-card>
...
export default {
  ...
  created: function () {
    ...
    this.getBusiness();
    this.correctBusiness =
      this.businessID.toString() === this.$cookie.get("business-auth");
  },
  ...
  methods: {
    ...
    saveChanges: function () {
      let req = this.$data;
      axios
        .put(`/api/business/id/${this.businessID}`, req)
        .then(() => {
          this.newAddress = "";
          this.newPhone = "";
          this.editing = false;
        ...
        .catch((err) => {
          eventBus.$emit("update-business-error", err);
        });
    },
